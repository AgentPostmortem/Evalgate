...
```bash
npm install -D evalgate        # or: npm install -g evalgate
```
...
```bash
npx evalgate run suite.eval.yaml
```

Save a baseline, then compare later runs against it:

```bash
npx evalgate baseline suite.eval.yaml --out baseline.json
npx evalgate compare suite.eval.yaml --base baseline.json --tolerance 0.01
```
...
> `npx evalgate run examples/support-agent.eval.yaml`
